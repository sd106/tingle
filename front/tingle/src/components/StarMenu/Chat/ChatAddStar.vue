...
const getFolloings = async (): Promise<void> => {
  try {
    ...
    window.alert(folloingInfo.value);
...
const getChatStars = async (): Promise<void> => {
  ...
  window.alert(chatStarsInfo.value);
...
// 스타 추가하기
const addStarChating = async (id: number): Promise<void> => {
try {
  ...
  if(folloingInfo?.value.length == 5) {
    ...
    const res = await axios.get(`${store.API_URL}/chatRoom/addTheStar/${myid}/${id}`);

    console.log(`${res.data.message}: ${res.data.code}`);

    // 시작 메시지 DB 에 저장
    addStarChatingMessages(id);
...
// 시작 메시지 DB 에 저장
const addStarChatingMessages = async (id: number): Promise<void> => {
try {
  const dataToSend = {
    userId: 30,
    direction: 1,
