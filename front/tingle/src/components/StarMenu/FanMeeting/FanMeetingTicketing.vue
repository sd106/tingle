  ...
  <div>
    <div class="row" style="height: 500px">
      <div class="col-md-8 p-0">
        <div
          class="img-fluid h-100 opacity-75"
          :style="{ backgroundImage: `url(${fanMeetingInfo.imgURL1})` }"
          style="background-size: cover"
        ></div>
      </div>
      <div
        v-if="!props.haveTicket && remainTicket > 0"
        class="col-md-4 rounded-3 d-flex flex-column justify-content-center bg-light p-4"
      >
        <div>
        ...
        <p>
          {{ store.formatDateTime(fanMeetingInfo.ticketingStartAt) }} ~
          {{ store.formatDateTime(fanMeetingInfo.ticketingEndAt) }}
        </p>
...
const getTicketNumber = async () => {
  try {
    ...
    const response = await axios.get(
      `https://i10d106.p.ssafy.io/api/fanMeeting/${props.fanMeetingInfo.id}/reservation`
    )
