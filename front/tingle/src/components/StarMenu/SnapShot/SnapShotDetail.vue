...
const deleteSnapshot = async (id: number) => {
  ...
  if (id && props.selectedSnapshot) {
    const response = await axios.delete(`http://i10d106.p.ssafy.io/api/snapshot/${id}/delete`)
...
const postComment = async () => {
  try {
    ...
    await axios.post(
      `http://i10d106.p.ssafy.io/api/snapshot/${props.selectedSnapshot?.snapshotId}/comment/new`,
      {
...
const deleteComment = async (id: number) => {
  try {
    await axios.post(
      `http://i10d106.p.ssafy.io/api/snapshot/${props.selectedSnapshot?.snapshotId}/comment/${id}/delete`
    )
    // 댓글 목록을 다시 불러오는 로직 필요
    router.go(0)
  } catch (error) {
