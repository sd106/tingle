...
const like = async (id: number, username: string) => {
  if (id) {
    try {
      // 좋아요 API 호출
      await axios.post(
        `http://i10d106.p.ssafy.io:8080/api/snapshot/${id}/likes`,
        { username: username },
...
const submitCommentEdit = async (commentId: number) => {
  try {
    const response = await axios.post(
      `http://i10d106.p.ssafy.io:8080/api/snapshot/${props.selectedSnapshot?.snapshotId}/comment/${commentId}/update`,
      {
