...
const like = async (id: number, username: string) => {
  if (id) {
    try {
      // 좋아요 API 호출
      await axios.post(
        `http://i10d106.p.ssafy.io/api/snapshot/${id}/likes`,
        { username: username },
...
const dislike = async (id: number, username: string) => {
  if (id) {
    try {
      // 좋아요 API 호출
      await axios.post(
        `http://i10d106.p.ssafy.io/snapshot/${id}/dislikes`,
        { username: username },
        { withCredentials: true }
      )
      console.log('싫어요 실행됨')
...
const deleteSnapshot = async (id: number) => {
  ...
  if (id && props.selectedSnapshot) {
    const response = await axios.delete(`http://i10d106.p.ssafy.io/snapshot/${id}/delete`)
...
const postComment = async () => {
  try {
    ...
    await axios.post(
      `http://i10d106.p.ssafy.io/snapshot/${props.selectedSnapshot?.snapshotId}/comment/new`,
      {
...
const submitCommentEdit = async (commentId: number) => {
  try {
    const response = await axios.post(
      `http://i10d106.p.ssafy.io/api/snapshot/${props.selectedSnapshot?.snapshotId}/comment/${commentId}/update`,
      {
...
const deleteComment = async (id: number) => {
  try {
    await axios.post(
      `http://i10d106.p.ssafy.io/snapshot/${props.selectedSnapshot?.snapshotId}/comment/${id}/delete`
    )
    // 댓글 목록을 다시 불러오는 로직 필요
    router.go(0)
  } catch (error) {
