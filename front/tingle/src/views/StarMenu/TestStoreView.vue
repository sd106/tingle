...
const getProductsByStarName = async (starName: string) => {
  try {
    const response = await axios.get(`http://i10d106.p.ssafy.io:8080/api/product/getByStarName/${starName}`);
    if (response.data.resultCode === 'SUCCESS') {
...
const getProductdelete = async (productId: number) => {
  try {
    const response = await axios.post(`http://i10d106.p.ssafy.io:8080/api/product/delete/${productId}`);
    if (response.data.resultCode === 'SUCCESS') {
...
const createProduct = async (productInfo: any, fileInputs: FileList) => {
  try {
    ...
    const response = await axios.post('http://i10d106.p.ssafy.io:8080/api/product/create', formData, {
      headers: {
...
const updateProduct = async (productIdupdate: number, updatedProductDto: any) => {
  try {
    const response = await axios.post(`http://i10d106.p.ssafy.io:8080/api/product/update/${productIdupdate}`, updatedProductDto);
...
const updateProductImage = async (productIdimageupdate: number, existingImageUrl: string, file: File) => {
  try {
    ...
    const response = await axios.post(`http://i10d106.p.ssafy.io:8080/api/product/updateImg/${productIdimageupdate}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    });
...
const uploadImage = async (productId: number) => {
  try {
    ...
    // Axios 요청 보내기
    const response = await axios.post(`http://i10d106.p.ssafy.io:8080/api/s3/upload/${productId}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
const updateImage = async (productId: number, imageId: number) => {
  try {
    ...
    // Axios 요청 보내기
    const response = await axios.post(`http://i10d106.p.ssafy.io:8080/api/s3/update/${productId}/${imageId}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
const deleteImage = async (imageId: number, productId: number) => {
  try {
    await axios.post(`http://i10d106.p.ssafy.io:8080/api/s3/delete/${imageId}`);
    getProductById(productId)
  } catch (error) {
