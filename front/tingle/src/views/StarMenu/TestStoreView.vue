...
const updateProduct = async (productIdupdate: number, updatedProductDto: any) => {
  try {
    const response = await axios.post(
      `http://i10d106.p.ssafy.io:8080/api/product/update/${productIdupdate}`,
      updatedProductDto
    )
...
const updateProductImage = async (
  ...
  try {
    ...
    const response = await axios.post(
      `http://i10d106.p.ssafy.io:8080/api/product/updateImg/${productIdimageupdate}`,
      formData,
      {
        headers: {
          'Content-Type': 'multipart/form-data'
...
const updateImage = async (productId: number, imageId: number) => {
  try {
    ...
    // Axios 요청 보내기
    await axios.post(`http://i10d106.p.ssafy.io:8080/api/s3/update/${productId}/${imageId}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    })
