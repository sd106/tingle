...
<!-- //-----------상품 생성----------------------------// -->
  ...
  <div class="tw-p-8">
    ...
    <div class="tw-grid tw-gap-4 tw-mb-8">
      <input type="text" v-model="productcreate.starName" placeholder="스타 이름" class="tw-input tw-input-bordered tw-w-full">
      <input type="text" v-model="productcreate.name" placeholder="상품 이름" class="tw-input tw-input-bordered tw-w-full">
      <input type="number" v-model.number="productcreate.amount" placeholder="수량" class="tw-input tw-input-bordered tw-w-full">
      <input type="number" v-model.number="productcreate.price" placeholder="가격" class="tw-input tw-input-bordered tw-w-full">
    ...
    <div class="tw-grid tw-grid-cols-3 tw-gap-4">
      <div v-for="(file, index) in previewFiles" :key="index" class="tw-relative tw-mb-4">
        ...
        <button @click="removeFile(index)" class="tw-btn tw-btn-error tw-btn-sm tw-absolute tw-right-0 tw-top-0">삭제</button>
      </div>
    </div>
...
const updateImage = async (productId: number, imageId: number) => {
  try {
    ...
    // Axios 요청 보내기
    await axios.post(`http://localhost:8080/s3/update/${productId}/${imageId}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
