...
const getHotStars = async () => {
  const response = await axios.get('http://i10d106.p.ssafy.io:8080/api/follow/hotStars');
...
const getTop10Stars = async () => {
  const response = await axios.get('http://i10d106.p.ssafy.io:8080/api/star/Top10Stars');
  hotStarsInfo.value = response.data.data;
  console.log(response);
}

const getStarsByCategory = async (category: number) => {
  const response = await axios.get(`http://i10d106.p.ssafy.io:8080/api/star/${category}`);
