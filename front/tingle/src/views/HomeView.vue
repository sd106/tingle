...
const getHotStars = async () => {
  const response = await axios.get('https://i10d106.p.ssafy.io/api/follow/hotStars')
...
const getTop10Stars = async () => {
  ...
  const response = await axios.get('https://i10d106.p.ssafy.io/api/star/Top10Stars')
  hotStarsInfo.value = response.data.data
  console.log(response)
}

const getStarsByCategory = async (category: number) => {
  const response = await axios.get(`https://i10d106.p.ssafy.io/api/star/${category}`)
