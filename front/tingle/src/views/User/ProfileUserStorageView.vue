...
import { useRoute } from 'vue-router'
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://i10d106.p.ssafy.io/api/order/getStarsPicture/${userId}`);
  ...
  .storage-image-container {
    flex: 1; /* 이미지 컨테이너가 사용 가능한 공간을 차지하되, */
  ...
  .storage-image {
    max-width: 100%; /* 너비를 100%로 설정하여 컨테이너에 맞게 조정 */
    height: auto; /* 높이를 auto로 설정하여 비율을 유지 */
    object-fit: cover; /* 컨테이너에 맞추되, 비율 유지 */
  ...
  .delete-button {
    position: absolute; /* 버튼을 상위 컨테이너에 대해 절대 위치 */
    bottom: 10px; /* 상단에서부터의 거리 */
    ...
    position: absolute; /* 버튼을 상위 컨테이너에 대해 절대 위치 */
    bottom: 15px; /* 하단에서부터의 거리 조정 */
    right: 15px; /* 오른쪽에서부터의 거리 조정 */
