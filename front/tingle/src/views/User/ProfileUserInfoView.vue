<template>
  ...
  <div style="width: 500px; height: 650px; position: relative;" v-if="fanState!.picture" class="p-4 d-flex flex-column justify-content-center align-items-center border">
    <img class="mb-5 mx-0 profile-pic" :src="fanState!.picture" alt="" style="max-height: 80%; object-fit: contain; width: auto;">
...
const deleteImage = async (fanId: number) => {
  try {
    const response = await axios.post(
      `http://localhost:8080/api/user/profilePicture/delete/${fanId}`
    )
    fanState!.picture = response.data.data
    if (response.data.resultCode === 'SUCCESS') {
