...
const starSignup = async () => {
  try {
    const response = await axios.post('http://i10d106.p.ssafy.io:8080/auth/member/signup', signupData, { withCredentials: true })
...
const loadMemeber = async () => {
  try {
    const response = await axios.get('http://i10d106.p.ssafy.io:8080/auth/member/info', { withCredentials: true })
    console.log(response)
